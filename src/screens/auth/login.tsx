...
import { useLogin, useResendOTP } from '@/lib/hooks/useAuth';
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  const { mutate: login, isPending, isSuccess, data: loginData } = useLogin();
  ...
  React.useEffect(() => {
    if (isSuccess && loginData) {
      ...
      if (!loginData.user.isEmailVerified) {
        // Automatically request a new OTP for the user
        resendOTP(loginData.user.email);
  ...
  }, [isSuccess, loginData, onLoginSuccess, resendOTP]);
  ...
  // OTP Verification Screen (shown when user is not verified)
  if (showOTPScreen && email) {
    return (
      <OTPVerification
        email={email}
        onBack={() => {
        ...
        subtitle="Please verify your email to continue. We've sent a verification code to"
      />
    );
  }
